...
export function CreateAccountPage() {
  ...
  const submitFormData = async (event) => {
    ...
    setFormError(comparePasswords(accountFormData.firstField, accountFormData.secondField))
  ...
  return (
    <Flex sx={LoginFlex} align={"center"}>
      <Stack align={"center"}> 
        <Heading fontSize={"3x1"} py={3}>Create an Account</Heading>
        <Text fontSize={"lg"}>Already have an account? Log in {" "}
          <Link to="/createaccount" color="#FDCA40">here</Link>
        </Text>
      </Stack>
      <Box sx={LoginBox}>
        <Stack as="form" spacing={3} onSubmit={submitFormData}>
